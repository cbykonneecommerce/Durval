...
$( document ).ready(function() {
    ...
    .then(response => {
        ...
        response.forEach(element => {
            ...
            $(".deptos ul").append( `<li class="depto-${element.name}"><a href="${element.url}?O=OrderByScoreASC">${divtext}</a></li>`);
            ...
            $(".sidenav").append(`<div style="display:block;border-top: solid 1px #E4E5E9;"><span class="depto-${element.name}"><a href="${element.url}?O=OrderByScoreASC">${divtext}</a></span><button class="dropdown-btn" id="${element.name}"><i class="fa fa-angle-down"></i></button></div>
           ...
           $(`.dropdown-btn#${element.name}`).toggle(() => {
            ...
            element.children.forEach((subs, index) => {
                ...
                if( index < 4) {
                    $(`.depto-${element.name} .row .col-sm-3:nth-of-type(1) ul`).append( `<li><a href="${subs.url}?O=OrderByScoreASC">${subs.name}</a></li>`);
                } else if (index < 8) {
                    $(`.depto-${element.name} .row .col-sm-3:nth-of-type(2) ul`).append( `<li><a href="${subs.url}?O=OrderByScoreASC">${subs.name}</a></li>`);
                } else {
                    $(`.depto-${element.name} .row .col-sm-3:nth-of-type(3) ul`).append( `<li><a href="${subs.url}?O=OrderByScoreASC">${subs.name}</a></li>`);
                }

                $(`.dropdown-container#${element.name}`).append(`<a href="${subs.url}?O=OrderByScoreASC">${subs.name}</a>`)
